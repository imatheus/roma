...
async function run() {
  console.log('Iniciando o preenchimento automático das respostas...');
  for (const item of respostas) {
    try {
      const res = await fetch('http://localhost:3500/api/save', {
        method: 'POST',
      ...
      if (data.success) {
        console.log(`✅ Salvo: ${item.id}`);
      } else {
        console.error(`❌ Erro ao salvar ${item.id}:`, data.error);
      }
    } catch (error) {
      console.error(`❌ Erro de requisição para ${item.id}:`, error.message);
    }
  }

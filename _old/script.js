...
function parseMarkdownToQuestions(md) {
...
const questionsData = parseMarkdownToQuestions(markdownText);
let timeoutMap = {};
let currentPage = 1;
...
function renderForm() {
  ...
  const pageItems = questionsData.slice(startIdx, endIdx);

  pageItems.forEach(q => {
    if (q.section !== lastSection) {
      formHtml += `<h2 class="section-title">${q.section}</h2>`;
      lastSection = q.section;
    }
    
    // Parse descriptions markdown using marked
    const descHtml = window.marked ? marked.parse(q.desc) : q.desc;
    
    formHtml += `
      <div class="question-card" id="card-${q.id}">
        <div class="question-header">${q.title}</div>
        <div class="question-desc">${descHtml}</div>
        <textarea name="${q.id}" id="input-${q.id}" class="answer-box" placeholder="Escreva a definição aqui..."></textarea>
        <div class="status-msg" id="status-${q.id}"></div>
      </div>
    `;
  });
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    paginationHtml += `<button class="page-btn ${activeClass}" onclick="mudarPagina(${i})">${i}</button>`;
  }
  paginationHtml += '</div>';
